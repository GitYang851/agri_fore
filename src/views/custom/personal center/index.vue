      ...
      <el-row :gutter="20">
        <el-col :span="6" v-for="item in favorites" :key="item.id">
          <el-card class="product-card">
            <img
              :src="`/dev-api/product/downloadPicture/${item.productId}`"
              class="product-image"
              @click="toProductDetail(item.productId)"
            />
            <h4>{{ item.productName }}</h4>
            <p class="price">￥{{ item.price }}</p>
            <el-button type="danger" size="small" @click="removeFavorite(item.productId)">
              取消收藏
            </el-button>
          </el-card>
        </el-col>
...
import { getcollectdata, removecollect } from '@/api/product2'

import { useRouter } from 'vue-router'
const router = useRouter()
const initData = () => {
  getcollectdata().then((response) => {
...
const removeFavorite = async (id) => {
  try {
    await ElMessageBox.confirm(`确定要取消收藏 吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
    await removecollect({ productId: id }).then((response) => {
      if (response.data >= 0) {
